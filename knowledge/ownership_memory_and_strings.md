...
`Vec<T>` の `v[i]` も同じく範囲外なら panic します。
実装上は `Vec<T>` の添字アクセスが slice `&[T]` の添字アクセスに委譲され、slice 側で `i < len` の境界チェックが入ります。
範囲内なら要素への参照を返し、範囲外なら `panic_bounds_check` 経由で `index out of bounds` の panic になります。
panic させずに範囲外を扱いたい場合は `v.get(i)` を使い、戻り値の `Option<&T>` を処理します。
