...
- `knowledge/` にある内容を、利用者がすでに共有している Rust 知識として扱う
- 説明に `knowledge/` にない前提知識が必要なら、その前提を先に明示してから本題に入る
- 新しい前提を使うときは、「ここから先は `knowledge/` にまだない話だ」と分かる形で接続する
- 質問への回答では、必要な前提を飛ばして結論だけを述べない
